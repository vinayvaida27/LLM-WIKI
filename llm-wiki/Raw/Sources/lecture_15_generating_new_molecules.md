---
Title: "Lecture 15 - Generating New Molecules"
Author: "MLCB24"
Reference: "[Lecture15 - Generating New Molecules - MLCB24](https://www.youtube.com/watch?v=VUmHMSME_BY&list=PLypiXJdtIca4gtioEPLIExlAKvu64z7rc&index=15)"
ContentType:
  - "markdown"
Created: 2026-05-24
Processed: true
...
VAEs have been adapted for generating **chemical structures** and **drug-like molecules**. HereÃ¢â‚¬â„¢s how:
...
P(X,Z)=P(ZÃ¢Ë†Â£X)Ãƒâ€”P(X)=P(XÃ¢Ë†Â£Z)Ãƒâ€”P(Z)

Here, P(ZÃ¢Ë†Â£X) represents the probability of a latent vector given the input, and P(XÃ¢Ë†Â£Z) is the probability of generating the input given the latent vector.

- **Challenge**: We donÃ¢â‚¬â„¢t know P(ZÃ¢Ë†Â£X) or P(X) directly. This is because we donÃ¢â‚¬â„¢t have a way to model the entire data distribution explicitly.

Instead, we work with P(XÃ¢Ë†Â£Z), which is our decoder, and P(Z), which we define based on our assumptions about the latent space.
...
The decoder P(XÃ¢Ë†Â£Z) generates data points (e.g., images or molecules) from the latent representation. However, the quality of these samples depends on how well the latent space is structured:
...
- **Data Probability (P(X))**: This is the probability of observing any specific data point in the training set. However, it is difficult to estimate directly because it depends on the entire dataset and the decoderÃ¢â‚¬â„¢s learned behavior.
- **Decoder Output Probability (P(XÃ¢Ë†Â£Z))**: This is known and represents the likelihood of generating a specific data point given its latent vector. This is literally what the decoder computes during training.
- **Posterior Probability (P(ZÃ¢Ë†Â£X))**: This is the probability of the latent vector given the observed data. It is complex to compute directly because it requires knowledge of both P(X)P(X)P(X) and the entire data distribution. Instead, we approximate this using a neural network, which is known as the **encoder** in the VAE.

### 5\. Applying BayesÃ¢â‚¬â„¢ Rule

We can use BayesÃ¢â‚¬â„¢ rule to relate these probabilities:

P(ZÃ¢Ë†Â£X)=P(XÃ¢Ë†Â£Z)Ãƒâ€”P(Z) / P(X)

- **Posterior Approximation**: We approximate P(ZÃ¢Ë†Â£X) with a learned neural network (the encoder), denoted as q(ZÃ¢Ë†Â£X). This is crucial because computing the true posterior is intractable.
- **Evidence Lower Bound (ELBO)**: Instead of maximizing the likelihood of the data directly (which is hard to compute), we maximize a surrogate objective called the **Evidence Lower Bound (ELBO)**. This involves two main terms:
    ...
    2.  **KL Divergence**: Ensures that the learned posterior distribution q(ZÃ¢Ë†Â£X) is close to the prior P(Z), promoting a well-structured latent space.
...
This structured approach contrasts with basic autoencoders, where there is no guarantee that nearby points in the latent space correspond to similar outputs. The VAEÃ¢â‚¬â„¢s probabilistic framework helps ensure continuity and coherence in the generated samples.
...
- **KL Divergence Loss**: This regularizes the latent space by penalizing deviations of Q(zÃ¢Ë†Â£x)Q(z|x)Q(zÃ¢Ë†Â£x) from the prior P(z)P(z)P(z). It prevents the model from simply memorizing the data and encourages generalizability.
...
H(p)=Ã¢Ë†â€™Ã¢Ë†â€˜xp(x)logÃ¢ÂÂ¡p(x)H(p) = -\\sum_{x} p(x) \\log p(x)H(p)=Ã¢Ë†â€™xÃ¢Ë†â€˜Ã¢â‚¬â€¹p(x)logp(x)
...
The **Kullback-Leibler (KL) divergence**, or **relative entropy**, is a way to measure the difference between two probability distributions. Specifically, it quantifies how one probability distribution p(x)p(x)p(x) diverges from a reference distribution q(x)q(x)q(x). The KL divergence DKL(pÃ¢Ë†Â£Ã¢Ë†Â£q)D_{KL}(p || q)DKLÃ¢â‚¬â€¹(pÃ¢Ë†Â£Ã¢Ë†Â£q) is defined as:
DKL(pÃ¢Ë†Â£Ã¢Ë†Â£q)=Ã¢Ë†â€˜xp(x)logÃ¢ÂÂ¡p(x)q(x)D_{KL}(p || q) = \\sum_{x} p(x) \\log \\frac{p(x)}{q(x)}DKLÃ¢â‚¬â€¹(pÃ¢Ë†Â£Ã¢Ë†Â£q)=xÃ¢Ë†â€˜Ã¢â‚¬â€¹p(x)logq(x)p(x)Ã¢â‚¬â€¹
For continuous variables, this becomes an integral. Unlike entropy, the KL divergence is not symmetric, meaning that:
DKL(pÃ¢Ë†Â£Ã¢Ë†Â£q)Ã¢â€°Â DKL(qÃ¢Ë†Â£Ã¢Ë†Â£p)D_{KL}(p || q) \\neq D_{KL}(q || p)DKLÃ¢â‚¬â€¹(pÃ¢Ë†Â£Ã¢Ë†Â£q)=DKLÃ¢â‚¬â€¹(qÃ¢Ë†Â£Ã¢Ë†Â£p)
This asymmetry reflects the fact that DKL(pÃ¢Ë†Â£Ã¢Ë†Â£q)D_{KL}(p || q)DKLÃ¢â‚¬â€¹(pÃ¢Ë†Â£Ã¢Ë†Â£q) tells us **how much information is lost** when we use qqq to approximate ppp.
...
- It is always **non-negative**, meaning DKL(pÃ¢Ë†Â£Ã¢Ë†Â£q)Ã¢â€°Â¥0D_{KL}(p || q) \\geq 0DKLÃ¢â‚¬â€¹(pÃ¢Ë†Â£Ã¢Ë†Â£q)Ã¢â€°Â¥0. The divergence is zero if and only if p=qp = qp=q everywhere.
- It is **not symmetric**, so DKL(pÃ¢Ë†Â£Ã¢Ë†Â£q)Ã¢â€°Â DKL(qÃ¢Ë†Â£Ã¢Ë†Â£p)D_{KL}(p || q) \\neq D_{KL}(q || p)DKLÃ¢â‚¬â€¹(pÃ¢Ë†Â£Ã¢Ë†Â£q)=DKLÃ¢â‚¬â€¹(qÃ¢Ë†Â£Ã¢Ë†Â£p).
- It does not satisfy the **triangle inequality**, so it is not a true distance metric but rather a measure of **divergence**.

These properties make the KL divergence particularly useful for tasks where we want to **minimize the difference** between an observed data distribution and a model's prediction. For example, in variational autoencoders, we use KL divergence to ensure that the learned latent representation q(zÃ¢Ë†Â£x)q(z|x)q(zÃ¢Ë†Â£x) is close to a simple prior distribution p(z)p(z)p(z), typically a Gaussian.
...
- The **KL divergence term**, which acts as a regularizer, ensuring that the learned latent space distribution q(zÃ¢Ë†Â£x)q(z|x)q(zÃ¢Ë†Â£x) does not deviate too much from the prior distribution p(z)p(z)p(z).
...
- **P(xÃ¢Ë†Â£z)**: The likelihood of the data given a particular latent representation.
- **Q(zÃ¢Ë†Â£x)**: The variational approximation of the posterior, which serves as our encoder.
...
We start by taking an **average over the encoder distribution** Q(zÃ¢Ë†Â£x). This is essentially integrating over all possible values of z based on the encoder's output. Formally, we write:
EQ(zÃ¢Ë†Â£x)\[logP(x)\]
This expectation allows us to simplify the original equation, as P(x) itself does not depend on z. Thus, this integral is simply logÃ¢ÂÂ¡P(x).

### Decomposing the Evidence Lower Bound (ELBO)

We can decompose logÃ¢ÂÂ¡P(x) into three main terms by applying algebraic manipulations and leveraging the expectation over Q(zÃ¢Ë†Â£x):
logÃ¢ÂÂ¡P(x)=EQ(zÃ¢Ë†Â£x)\[logÃ¢ÂÂ¡P(xÃ¢Ë†Â£z)\]Ã¢Ë†â€™KL(Q(zÃ¢Ë†Â£x)Ã¢Ë†Â£Ã¢Ë†Â£P(z))+KL(Q(zÃ¢Ë†Â£x)Ã¢Ë†Â£Ã¢Ë†Â£P(zÃ¢Ë†Â£x))\\log P(x) = \\mathbb{E}\_{Q(z|x)}\[\\log P(x|z)\] - \\text{KL}(Q(z|x) || P(z)) + \\text{KL}(Q(z|x) || P(z|x))logP(x)=EQ(zÃ¢Ë†Â£x)Ã¢â‚¬â€¹\[logP(xÃ¢Ë†Â£z)\]Ã¢Ë†â€™KL(Q(zÃ¢Ë†Â£x)Ã¢Ë†Â£Ã¢Ë†Â£P(z))+KL(Q(zÃ¢Ë†Â£x)Ã¢Ë†Â£Ã¢Ë†Â£P(zÃ¢Ë†Â£x))
Let's break down these components:

- **Reconstruction Term** (EQ(zÃ¢Ë†Â£x)\[logÃ¢ÂÂ¡P(xÃ¢Ë†Â£z)\]\\mathbb{E}\_{Q(z|x)}\[\\log P(x|z)\]EQ(zÃ¢Ë†Â£x)Ã¢â‚¬â€¹\[logP(xÃ¢Ë†Â£z)\]):
...
- **KL Divergence between the Encoder Output and Prior** (KL(Q(zÃ¢Ë†Â£x)Ã¢Ë†Â£Ã¢Ë†Â£P(z))\\text{KL}(Q(z|x) || P(z))KL(Q(zÃ¢Ë†Â£x)Ã¢Ë†Â£Ã¢Ë†Â£P(z))):
...
- **KL Divergence between the Encoder Output and the True Posterior** (KL(Q(zÃ¢Ë†Â£x)Ã¢Ë†Â£Ã¢Ë†Â£P(zÃ¢Ë†Â£x))\\text{KL}(Q(z|x) || P(z|x))KL(Q(zÃ¢Ë†Â£x)Ã¢Ë†Â£Ã¢Ë†Â£P(zÃ¢Ë†Â£x))):
    This term quantifies the difference between the approximated posterior Q(zÃ¢Ë†Â£x)Q(z|x)Q(zÃ¢Ë†Â£x) (from the encoder) and the true posterior P(zÃ¢Ë†Â£x)P(z|x)P(zÃ¢Ë†Â£x). While this term is theoretically informative, it is not directly computable because P(zÃ¢Ë†Â£x)P(z|x)P(zÃ¢Ë†Â£x) is intractable. However, by minimizing the first two terms, we indirectly minimize this third term as well.
...
- - The first KL divergence term (KL(Q(zÃ¢Ë†Â£x)Ã¢Ë†Â£Ã¢Ë†Â£P(z))\\text{KL}(Q(z|x) || P(z))KL(Q(zÃ¢Ë†Â£x)Ã¢Ë†Â£Ã¢Ë†Â£P(z))) acts as a **regularization term**, ensuring that the learned latent distribution Q(zÃ¢Ë†Â£x)Q(z|x)Q(zÃ¢Ë†Â£x) does not diverge too far from the prior P(z)P(z)P(z). This maintains a smooth and consistent latent space.
    - The second KL divergence term (KL(Q(zÃ¢Ë†Â£x)Ã¢Ë†Â£Ã¢Ë†Â£P(zÃ¢Ë†Â£x))\\text{KL}(Q(z|x) || P(z|x))KL(Q(zÃ¢Ë†Â£x)Ã¢Ë†Â£Ã¢Ë†Â£P(zÃ¢Ë†Â£x))) indicates how well our encoder Q(zÃ¢Ë†Â£x)Q(z|x)Q(zÃ¢Ë†Â£x) approximates the true posterior P(zÃ¢Ë†Â£x)P(z|x)P(zÃ¢Ë†Â£x). While we cannot compute this directly, minimizing the other terms indirectly helps approximate it.
...
ELBO=EQ(zÃ¢Ë†Â£x)\[logÃ¢ÂÂ¡P(xÃ¢Ë†Â£z)\]Ã¢Ë†â€™KL(Q(zÃ¢Ë†Â£x)Ã¢Ë†Â£Ã¢Ë†Â£P(z))
...
**Step 2: Regularization:** The KL divergence term ensures that the latent space distribution Q(zÃ¢Ë†Â£x)Q(z|x)Q(zÃ¢Ë†Â£x) remains close to the prior P(z)P(z)P(z). This regularization prevents the model from overfitting and allows for smooth interpolation between points in the latent space.
...
In the context of variational autoencoders (VAEs), the **Evidence Lower Bound (ELBO)** is a key concept that helps us approximate the complex problem of maximizing the probability of the observed data. LetÃ¢â‚¬â„¢s break this down step by step:

1.  **Maximizing the Data Likelihood:**
    - The primary goal of generative modeling with a VAE is to maximize the likelihood of the observed data, denoted as logÃ¢ÂÂ¡P(X)\\log P(X)logP(X). However, directly computing this quantity is intractable because it involves integrating over the entire latent space ZZZ. This is where the evidence lower bound (ELBO) comes in as a useful proxy.
2.  **KL Divergence and the ELBO:**
    - The key to understanding the ELBO is recognizing the relationship between the actual data likelihood logÃ¢ÂÂ¡P(X), the encoder approximation Q(ZÃ¢Ë†Â£X), and the true posterior P(ZÃ¢Ë†Â£X).
    - We can express the data likelihood in terms of two components: the ELBO and an **encoder error** term that represents the KL Divergence between Q(ZÃ¢Ë†Â£X)(the approximate posterior) and P(ZÃ¢Ë†Â£X) (the true posterior).
    - The ELBO comprises two terms:
        - **Reconstruction Loss:** This term measures how well the decoder can reconstruct the input data from the latent space representation ZZZ. It captures the probability of the input data given the latent variables, logÃ¢ÂÂ¡P(XÃ¢Ë†Â£Z).
        - **KL Divergence Term:** This term measures the divergence between the approximate posterior Q(ZÃ¢Ë†Â£X) and the prior distribution P(Z). It effectively regularizes the latent space to ensure it follows a desired distribution (e.g., a standard Gaussian).
3.  **Maximizing the ELBO:**
    - Instead of directly maximizing logÃ¢ÂÂ¡P(X)\\log P(X)logP(X), which is difficult to compute, we maximize the ELBO. This is advantageous because:
        - **Maximizing the ELBO** implicitly increases the likelihood of the observed data logÃ¢ÂÂ¡P(X).
        - It simultaneously minimizes the KL Divergence between the approximate posterior Q(ZÃ¢Ë†Â£X) and the true posterior P(ZÃ¢Ë†Â£X), which reduces encoder error without requiring us to explicitly compute the intractable posterior.
4.  **Forces Acting on the Latent Variables (ÃŽÂ¼\\muÃŽÂ¼ and ÃÆ’\\sigmaÃÆ’):**
    - Within the ELBO, there are two opposing forces on the mean (ÃŽÂ¼\\muÃŽÂ¼) and variance (ÃÆ’\\sigmaÃÆ’) of the latent variables:
        - The **reconstruction loss** encourages ÃŽÂ¼\\muÃŽÂ¼ to map data points to specific locations in the latent space that allow for accurate reconstructions.
        - The **KL Divergence** term regularizes ÃŽÂ¼\\muÃŽÂ¼ and ÃÆ’\\sigmaÃÆ’ to match the prior distribution, typically a standard Gaussian centered around the origin. This prevents the latent variables from spreading out too much and ensures the learned representations are well-structured.
5.  **Balancing Reconstruction and Regularization:**
    - The model must balance two conflicting objectives:
        - **Accurate Reconstruction:** To minimize the reconstruction loss, the latent space representations (ÃŽÂ¼) should ideally be far apart for different data points (e.g., cats, cars, frogs), so they can be reconstructed accurately by the decoder.
        - **Latent Space Regularization:** To minimize the KL Divergence, the representations (ÃŽÂ¼ and ÃÆ’) should follow a standard Gaussian distribution, which may involve compressing different classes closer together in the latent space.
6.  **Fast Computation of KL Divergence:**
    - Since the prior distribution is chosen as a standard Gaussian (mean of 0, standard deviation of 1), the KL Divergence term can be computed efficiently using a closed-form expression. Given the predicted ÃŽÂ¼\\muÃŽÂ¼ and ÃÆ’\\sigmaÃÆ’ from the encoder, we use the formula for KL Divergence between two Gaussians, which simplifies the computation during training.
...
The KL Divergence term acts as a **regularization force**, pulling the learned mean (ÃŽÂ¼\\muÃŽÂ¼) of the latent variables towards the origin (i.e., a mean of zero). Additionally, it pushes the variance (ÃÆ’2\\sigma^2ÃÆ’2) towards one, enforcing a **unit Gaussian** structure for the latent space. This regularization encourages the latent space to be smooth and consistent across samples, making it easier to interpolate between different points in the space and sample new data effectively.
...
On the other hand, the reconstruction loss acts as a **competing force**. This term aims to accurately reconstruct the input data from the latent representation, driving the encoder to place different input examples into distinct regions of the latent space. Essentially, the reconstruction loss encourages the separation of ÃŽÂ¼\\muÃŽÂ¼ values in the latent space to ensure that different inputs are mapped to different parts of the space, preserving their unique characteristics.

If the reconstruction loss dominates, ÃÆ’2\\sigma^2ÃÆ’2 would ideally get very small, allowing the VAE to memorize the training set by creating highly precise, localized representations. This would lead to a highly specialized latent space but at the cost of **poor generalization**.
...
In summary, the opposing forces in a VAE Ã¢â‚¬â€ regularization via the KL Divergence and separation via reconstruction loss Ã¢â‚¬â€ create a delicate balance. The model trades off perfect reconstruction for a more structured and generalizable latent space, leading to slightly less precise but more versatile generative capabilities. This compromise is crucial for applications that require exploration of the latent space, such as generating new chemical structures or optimizing lead compounds in drug discovery.
...
In a VAE, we introduce a **stochastic sampling step**. When the encoder produces the latent variables, it estimates the **mean (ÃŽÂ¼\\muÃŽÂ¼)** and the **standard deviation (ÃÆ’\\sigmaÃÆ’)** of a Gaussian distribution. The latent representation is then sampled from this Gaussian distribution. This step is probabilistic, which poses a problem for backpropagation:
...
1.  Instead of directly sampling a value from the Gaussian distribution, we first generate a random value (ÃÂµ\\epsilonÃÂµ) from a standard normal distribution (mean = 0, variance = 1).
2.  We then transform this random value using the estimated mean (ÃŽÂ¼\\muÃŽÂ¼) and standard deviation (ÃÆ’\\sigmaÃÆ’): z=ÃŽÂ¼+ÃÆ’Ãƒâ€”ÃÂµz = \\mu + \\sigma \\times \\epsilonz=ÃŽÂ¼+ÃÆ’Ãƒâ€”ÃÂµ
3.  By doing this, the randomness is encapsulated in the ÃÂµ\\epsilonÃÂµ, which is independent of the model's parameters. The transformation (ÃŽÂ¼+ÃÆ’Ãƒâ€”ÃÂµ\\mu + \\sigma \\times \\epsilonÃŽÂ¼+ÃÆ’Ãƒâ€”ÃÂµ) is now a deterministic function of ÃŽÂ¼\\muÃŽÂ¼ and ÃÆ’\\sigmaÃÆ’.
4.  This transformation allows us to treat ÃŽÂ¼\\muÃŽÂ¼ and ÃÆ’\\sigmaÃÆ’ as standard parameters, which can be differentiated and optimized through backpropagation.

### Why the Reparameterization Trick Works

- The key idea here is that while we still introduce randomness, it is done **outside** the differentiable part of the model. The random noise (ÃÂµ\\epsilonÃÂµ) is generated independently and does not rely on the modelÃ¢â‚¬â„¢s parameters.
- By scaling and shifting this random noise using ÃŽÂ¼\\muÃŽÂ¼ and ÃÆ’\\sigmaÃÆ’, we can produce the desired sample without interrupting the flow of gradients.
...
However, the ideal scenario described above doesn't always naturally emerge. While the KL divergence term in the VAE loss function regularizes the latent space by enforcing a standard Gaussian prior, this alone may not always create a sufficiently disentangled or interpretable latent representation. This is where **ÃŽÂ²-VAEs** come into play.

### Introducing ÃŽÂ²-VAEs: Scaling the KL Divergence

The **ÃŽÂ²-VAE** is a variant of the VAE where we introduce a scaling factor, **ÃŽÂ²**, to the KL divergence term in the loss function. The standard VAE loss function is:

ELBO=E<sub>Q(zÃ¢Ë†Â£x)</sub>\[logÃ¢ÂÂ¡P(xÃ¢Ë†Â£z)\]Ã¢Ë†â€™KL(Q(zÃ¢Ë†Â£x)Ã¢Ë†Â¥P(z))

In a ÃŽÂ²-VAE, we modify this by introducing the **ÃŽÂ² parameter**:

ELBO<sub>ÃŽÂ²</sub>\=E<sub>Q(zÃ¢Ë†Â£x)</sub>\[logÃ¢ÂÂ¡P(xÃ¢Ë†Â£z)\]Ã¢Ë†â€™ÃŽÂ²Ã¢â€¹â€¦KL(Q(zÃ¢Ë†Â£x)Ã¢Ë†Â¥P(z))

#### Effect of the ÃŽÂ² Parameter:
...
3.  **When ÃŽÂ² < 1**:
    ...
    - As ÃŽÂ² approaches zero, the VAE effectively behaves like a **basic autoencoder**, where the primary focus is on minimizing reconstruction error without any imposed structure on the latent space.

### The Benefits of ÃŽÂ²-VAEs: Disentangled Latent Representations

The main advantage of using ÃŽÂ²-VAEs is the potential to learn a **disentangled latent space**. In a disentangled representation, each dimension of the latent space captures an **independent factor of variation** in the data. For example, in the case of facial images:
...
In standard VAEs (ÃŽÂ² = 1), the latent dimensions may not have a clear, interpretable meaning. Nearby points in the latent space might correspond to drastically different inputs, such as the transition from one person's face to another's. The lack of clear structure can make the interpolation between data points less meaningful.

In contrast, ÃŽÂ²-VAEs (with ÃŽÂ² > 1) tend to produce a **more organized latent space**. When visualizing the latent dimensions, we often see smoother transitions. For example, in generative models of faces:
...
While increasing ÃŽÂ² encourages a disentangled and structured latent space, there are trade-offs:

- **Loss of Reconstruction Accuracy**: When ÃŽÂ² is increased beyond 1, the model places less emphasis on accurately reconstructing the input data. As a result, the reconstructed samples may appear less precise or more blurry.
- **Balance Between Fidelity and Generalization**: ÃŽÂ²-VAEs achieve a balance between memorizing the training data (as in autoencoders) and generalizing well to unseen data (as in VAEs). The optimal choice of ÃŽÂ² depends on the application: for tasks requiring strong generalization and disentangled features, a higher ÃŽÂ² may be preferable.

### Visualizing the Latent Space

When comparing the latent spaces of a standard VAE versus a ÃŽÂ²-VAE:

- In a **standard VAE**, the latent space often appears cluttered, with no clear organization or interpretation of dimensions.
- In a **ÃŽÂ²-VAE**, the latent space is more structured. Each dimension might clearly correspond to a distinct, meaningful feature of the input data, making it easier to explore variations systematically.

For example, in image generation tasks, if we traverse the latent space of a ÃŽÂ²-VAE along one dimension, we might observe a smooth change in head orientation (e.g., a face turning left to right). In a standard VAE, however, the same traversal might result in a random morphing of facial features, making it difficult to interpret the changes.

### Summary: Why Use ÃŽÂ²-VAEs?

ÃŽÂ²-VAEs provide a simple yet powerful modification to standard VAEs that **encourages disentangled representations** and a more structured latent space. By adjusting the weight of the KL divergence term, we can control the trade-off between reconstruction accuracy and latent space regularization:

- **Higher ÃŽÂ² values** lead to more regularization, better disentanglement, and smoother, more interpretable latent spaces.
- **Lower ÃŽÂ² values** prioritize reconstruction fidelity, but may result in less meaningful or entangled latent dimensions.

In practice, the choice of ÃŽÂ² depends on the specific needs of the task. For applications like **drug discovery**, where exploring the latent space to generate new candidate molecules is crucial, a ÃŽÂ²-VAE with a higher ÃŽÂ² can help provide a clearer and more navigable latent representation, enabling researchers to systematically explore variations and identify promising leads.

This concludes our discussion on ÃŽÂ²-VAEs. Next, we will shift our focus to another generative model that has revolutionized the field of generative modeling: **Generative Adversarial Networks (GANs)**.
...
2.  **Discriminator (D)**:
    ...
    - The discriminatorÃ¢â‚¬â„¢s objective is to correctly identify whether a given sample is real or fake.
...
Recently, **diffusion models** have gained prominence, often outperforming GANs in generating high-quality, diverse samples. While we wonÃ¢â‚¬â„¢t go into diffusion models in detail in this lecture, itÃ¢â‚¬â„¢s important to note that they offer a different approach, based on gradually transforming noise into a structured sample through a sequence of iterative steps.
...
This instability and lack of a coherent mapping between similar molecules in sequence space hinder the modelÃ¢â‚¬â„¢s ability to generate valid, chemically meaningful structures. Generative models operating directly on SMILES strings often produce invalid or unrealistic molecules, as the space of valid chemical structures is sparsely represented in the sequence domain.
...
- This problem becomes pronounced when making small modifications to existing molecules. For instance, changing a single methyl groupÃ¢â‚¬â„¢s position might require traversing through multiple invalid intermediates.
...
1.  **Graph Encoder-Decoder**:
    - The **graph encoder** takes a molecular graph as input and compresses it into a **latent space representation**, capturing the global structure of the molecule. This representation includes a mean vector (ÃŽÂ¼\\muÃŽÂ¼) and a standard deviation vector (ÃÆ’\\sigmaÃÆ’), as in a standard VAE.
2.  **Junction Tree Encoder-Decoder**:
    ...
    - The **junction tree encoder** captures the hierarchical structure of the molecule by encoding the relationships between these substructures, again producing a latent space representation (ÃŽÂ¼\\muÃŽÂ¼ and ÃÆ’\\sigmaÃÆ’) for the tree.
...
These two embeddings are **concatenated** before being fed into the VAEÃ¢â‚¬â„¢s decoder. This dual representation allows the model to capture both fine-grained atomic details and broader structural patterns, enabling more accurate generation of chemically valid molecules.
...
- It introduces a hierarchical decomposition of molecules into **substructures**, enhancing the modelÃ¢â‚¬â„¢s ability to generate valid and diverse chemical entities.
...
In this final section of the chapter, we explore how advanced AI techniques, particularly those involving **molecular graph representations**, have been applied to the field of **antibiotic discovery**. This work is part of a collaborative effort between researchers at MIT and the Broad Institute, including notable contributions from Jim CollinsÃ¢â‚¬â„¢s lab in the Institute for Medical Engineering and Science (IMES). The approach leverages graph neural networks and novel message-passing strategies to accelerate the identification of promising antibiotic candidates.
...
To validate the modelÃ¢â‚¬â„¢s predictions, the researchers conducted further experimental testing:
...
2.  **Results**: Nearly **50% of the top predictions** were found to inhibit _E. coli_ growth, a significant improvement compared to random screening, which typically yields much lower hit rates. This highlights the modelÃ¢â‚¬â„¢s ability to accurately predict antibiotic activity and prioritize promising candidates for further investigation.
...
- **In Vivo Efficacy**:
    - The researchers further validated HalicinÃ¢â‚¬â„¢s efficacy in a **mouse model** of infection, where it showed significant activity against _Clostridioides difficile_ (commonly known as _C. diff_), a major cause of hospital-acquired gut infections.
