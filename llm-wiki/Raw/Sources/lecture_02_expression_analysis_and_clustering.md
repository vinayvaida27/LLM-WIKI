---
Title: "Lecture 2: Expression Analysis and Clustering"
Author: "MLCB24"
Reference: "[Lecture02 - Expression Analysis Clustering Classification - MLCB24](https://www.youtube.com/watch?v=Air6zIVf0M8&list=PLypiXJdtIca4gtioEPLIExlAKvu64z7rc&index=2)"
ContentType:
  - "markdown"
Created: 2026-05-24
Processed: true
...
Welcome to Lecture 2! I hope everyone is feeling great today. Just a quick reminderâ€”please fill out the first day survey by Friday. Weâ€™re organizing teams for the formal mentoring session, and this survey will help us understand you better and form balanced groups. The more information we have, the better we can structure these teams for the mentoring session on Friday, where you will also be completing a one-page profile. This will help your classmates get to know you and facilitate more effective collaboration.
...
**Gaussian Mixture Models (GMMs)**: Weâ€™ll build on K-means to understand GMMs, which provide a probabilistic approach to clustering that accounts for the inherent variability within data.
...
**Next Week**: We dive into **Gene Regulation**â€”exploring the regulatory motifs, signals of histone modifications, and the broader regulatory landscape that drives gene expression. This will also involve methods to model these signals computationally.

**Single-Cell Genomics**: While todayâ€™s lecture will touch on the basics of clustering and classification, we will dedicate a future lecture to the nuances of single-cell genomics and spatial transcriptomics. These rapidly evolving technologies are reshaping our understanding of cellular heterogeneity and gene expression in situ.
...
- **Structured Output**: Generating organized, meaningful information from raw, unstructured data. For example, translating a doctorâ€™s spoken diagnosis into structured fields like â€œtumor size: 7â€ or â€œdiabetes stage: severe.â€

Structured data is organized and quantifiable, while **unstructured data**â€”like free text or imagesâ€”lacks such format. Machine learning has traditionally focused on structured data, but this distinction is blurring as techniques for processing unstructured data (e.g., clustering on text or images) have advanced.
...
**Denoising**: Identifying and removing noiseâ€”unwanted variability in data that can obscure the true signal, such as technical artifacts rather than biological differences.

Across all these tasks, the key is to improve performance with more data. This is the essence of machine learning. To measure improvement, we need a performance metric that quantifies the modelâ€™s accuracy or effectiveness.
...
**Inference** is the process of deducing hidden states based on observed data. For example, looking outside and seeing bright sunlight might lead you to infer itâ€™s summer. If you see snow, you infer itâ€™s winter. The observations are directâ€”light, rain, snowâ€”but the hidden states are inferred: the season, the weather system, or any other underlying condition.
...
Very often, we can directly measure and express **forward probabilities**â€”the likelihood of observable events given a known state of the world. For example: **Winter** might have a 60% chance of snow. **Summer** might have a 60% chance of sunshine. These are relatively easy to measure because they start from a known condition and predict observable outcomes.

However, **inferences** work in the reverse direction. They aim to determine the hidden state given the observations: **Given itâ€™s raining, whatâ€™s the probability of a storm? Given high expression of a particular gene, whatâ€™s the probability of Alzheimer's?**

Forward probabilities are often described as **generative probabilities** because they model how known states of the world generate observable data. For instance, if you know itâ€™s summer, you can predict temperatures and weather patterns typical of that season.
...
#### Using Bayesâ€™ Rule for Inference
...
- **Forward Probability (Generative)**: Probability of observing data given a hypothesis, P(Dataâˆ£Hypothesis)P(\\text{Data} \\mid \\text{Hypothesis})P(Dataâˆ£Hypothesis).
- **Reverse Inference**: Probability of the hypothesis given the data, P(Hypothesisâˆ£Data)P(\\text{Hypothesis} \\mid \\text{Data})P(Hypothesisâˆ£Data).
...
#### Understanding Bayesâ€™ Rule in Simple Terms
...
1.  Multiply the fraction of the world that is blue (P(B)P(B)P(B)) by the fraction of the blue that is also red (P(Aâˆ£B)P(A \\mid B)P(Aâˆ£B)).
2.  Multiply the fraction of the world that is red (P(A)P(A)P(A)) by the fraction of the red that is also blue (P(Bâˆ£A)P(B \\mid A)P(Bâˆ£A)).

In both cases, the purple area remains the same, demonstrating how these probabilities are interconnected.

#### Applying Bayesâ€™ Rule to Inference

In the context of Bayesian inference, the formula helps us derive what we call the **posterior probability**:

- **Posterior Probability** (P(Hypothesisâˆ£Data)P(\\text{Hypothesis} \\mid \\text{Data})P(Hypothesisâˆ£Data)): The probability of the hypothesis after observing the data.
- **Likelihood** (P(Dataâˆ£Hypothesis)P(\\text{Data} \\mid \\text{Hypothesis})P(Dataâˆ£Hypothesis)): The probability of observing the data given the hypothesis.
...
This marginal probability is calculated as a weighted sum of the likelihoods across all hypotheses. It helps normalize the results but doesnâ€™t change the relative ranking of different hypotheses.
...
Consider taking a test for a rare disease. If the test is positive, is it more likely that you have the disease or that it was a false positive? Often, due to the low prior probability of the disease, the latter is trueâ€”you might not have the disease even with a positive result. This highlights the crucial role of priors in Bayesian inference, especially for rare conditions.
...
- In a real-world problem, we donâ€™t know the underlying distributions, centroids, or spreads.
- **Clustering** attempts to infer the hidden distributions without any labeled guidance.

### Supervised vs. Unsupervised Learning

- **Unsupervised Learning**: Finds patterns or structures in data without labels. For example, inferring clusters when you donâ€™t know the green, red, or blue labels.
...
These two main tasksâ€”**clustering (unsupervised)** and **classification (supervised)**â€”are central to the lecture. For example, in genomics, you may measure gene expression in different tissues (like brain and liver) and attempt to classify or cluster the data based on these measurements.
...
**Clustering**: Groups data points based on proximity without predefined labels, identifying structure and patterns within the data. Itâ€™s challenging to validate because it lacks explicit performance metrics and often requires external validation.
...
**Representation Learning**: The process of discovering underlying features or patterns that best represent the dataâ€™s true nature, such as latent variables like disease states or demographic factors.
...
In this lecture, itâ€™s crucial to differentiate between various concepts like AI, Machine Learning (ML), Representation Learning, and Generative AI. Letâ€™s explore these concepts in detail.
...
- **Old Paradigm**: Tasks like chess, logic, and planning were seen as the pinnacle of intelligenceâ€”hard for humans, but structured and rule-based, making them easier for machines.
...
**Layers of Abstraction**: Similar to how the human brain processes information, deep learning uses layers of neurons (real or artificial) to recognize increasingly complex patternsâ€”from edges to shapes to complete objects.

**Neocortex Expansion**: Evolutionarily, humans have greatly expanded the neocortex, enabling us to perform complex reasoning, sensory processing, and abstract thinkingâ€”all of which are mirrored in the architecture of deep learning models.
...
Gene expression data can be analyzed in two main ways: comparing expression profiles across genes or across conditions (e.g., different patients or treatments). In the context of disease, such as Alzheimerâ€™s, clustering can reveal how gene expression patterns differ between affected and unaffected individuals, helping to identify disease-associated genes.
...
4.  **Iteration**: These steps are repeatedâ€”reassigning points and recalculating centroidsâ€”until convergence, which occurs when there are no further changes in the assignments of points to clusters or when the centroids stop moving significantly.
...
where CkC_kCkâ€‹ denotes the set of points assigned to cluster kkk, and Î¼k\\mu_kÎ¼kâ€‹ is the centroid of that cluster. The algorithmâ€™s iterative process of assigning points and updating centroids is specifically designed to minimize this cost, thereby improving the quality of the clustering.
...
However, GMMs go beyond simple cluster centersâ€”they incorporate different spreads and variances, allowing for clusters of different sizes and shapes. This is particularly useful when clusters overlap or have elongated shapes, which are not well captured by K-means.
...
The basic process of hierarchical clustering involves iteratively merging the closest pairs of points or clusters. Hereâ€™s how it unfolds:
...
**Visual Insights**: The dendrogram offers a clear visual representation of how clusters merge, providing insights into the dataâ€™s structure at multiple levels.
...
**Time Complexity**: The approach can be computationally intensive, particularly when calculating all pairwise distances (O(NÂ²)). Using centroid linkage can reduce some of this computational burden but requires recalculating centroids each time clusters merge.

### Applications and Evaluation

Hierarchical clustering is widely used in various fields, such as biology (e.g., gene expression analysis) and market segmentation, where itâ€™s helpful to see data grouped naturally. To evaluate the results, you can compare clusters with external labels or additional measurements to assess how well the clustering captures meaningful patterns.
...
One example involves clustering survey responses about personal experiences. Responses like "I have a relationship that did not go as expected" or "I struggled with personal setbacks" can be clustered based on semantic similarity. This approach reveals common psychological or emotional themes that arenâ€™t captured by standard psychiatric categories defined decades ago. It provides a more personalized understanding of human experiences, showing that individual struggles often share common threads, even if they donâ€™t fit neatly into traditional categories.
...
**Media Content Analysis**: Clustering text goes beyond scientific data. For instance, clustering topics from thousands of podcast transcripts, like Lex Fridmanâ€™s discussions, allows one to visualize thematic landscapes. You might find that philosophical discussions, including "the meaning of life," naturally cluster near other profound topics, revealing the underlying structure of content in a way thatâ€™s immediately accessible and engaging.
...
Using these probabilities, you can define a **discriminant function**â€”a threshold to classify a point based on which class probability is higher. This decision boundary can be tuned by adjusting the log probabilities or raw probabilities until it accurately separates the classes.
...
## [1:20:50](https://www.youtube.com/watch?v=Air6zIVf0M8&list=PLypiXJdtIca4gtioEPLIExlAKvu64z7rc&index=2&t=4850s) [[lecture_02_summary]]Summary
