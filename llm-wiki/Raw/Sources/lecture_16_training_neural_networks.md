---
Title: "Lecture 16 - Training Neural Networks"
Author: "MLCB24"
Reference: "https://www.youtube.com/watch?v=ea0g2gS6YLE&list=PLypiXJdtIca4gtioEPLIExlAKvu64z7rc&index=17"
ContentType:
  - "markdown"
Created: 2026-05-24
Processed: true
...
## [0:00](https://www.youtube.com/watch?v=ea0g2gS6YLE&t=0s) Training Neural Networks â€“ A Case Study on Blood-Brain Barrier Prediction

In this chapter, weâ€™ll explore the step-by-step process of **training a neural network model** to predict the ability of small molecules to cross the **blood-brain barrier (BBB)**. This hands-on example, which draws on real-world data and machine learning practices, will provide a detailed overview of model training, data handling, and optimization strategies essential in practical applications.
...
The data includes **SMILES (Simplified Molecular Input Line Entry System) strings**, which provide a compact representation of a moleculeâ€™s structure. To train our model, we need to transform these strings into a numerical format that a neural network can interpret.
...
2.  **SMILES to Embeddings**: Directly using SMILES strings as input to the neural network isnâ€™t feasible. Instead, we transform them into **fixed-length vector embeddings** using a pre-trained model (e.g., **KMTA from Hugging Face**). KMTA, based on the BERT architecture, outputs an embedding for each SMILES string.
3.  **Embedding Tokenization**: SMILES strings are tokenized before passing into KMTA. We use KMTAâ€™s tokenizer to segment SMILES strings into appropriate tokens, reducing the complexity of the embedding process.
...
Finally, we test our trained model on unseen data to gauge its ability to generalize. The test data provides insight into whether the modelâ€™s performance is due to genuine learning or overfitting.
...
At the core of neural network training lies **gradient descent**, a process where the modelâ€™s weights are iteratively adjusted to minimize a given **loss function**. Each adjustment aims to make the modelâ€™s predictions closer to the true labels. This process involves:
...
This approach works best in theory when applied over the entire dataset, often called **batch gradient descent**. However, for computational efficiency, we typically use **mini-batches**â€”small subsets of the dataâ€”to approximate the gradient. This practice, known as **stochastic gradient descent (SGD)**, has several advantages:
...
- **Advantages**:
    ...
    - **Stability on Rough Loss Surfaces**: For optimization landscapes with many small â€œbumps,â€ momentum helps the model avoid getting stuck, effectively smoothing out the optimization path.
...
- **Limitations**: Adagradâ€™s adaptive learning rate continuously decreases, which can sometimes cause the model to stop learning prematurely.
...
In PyTorch, optimization is handled by classes that encapsulate various strategies, making it easy to implement momentum or adaptive learning rates. Letâ€™s go over setting up a **PyTorch optimizer** with AdamW and weight decay:
...
- **Dropout**: Randomly â€œdrops outâ€ (sets to zero) neurons during training, preventing the model from relying too heavily on any specific set of neurons.
...
vt=Î²â‹…v<sub>tâˆ’1</sub>+(1âˆ’Î²)â‹…g<sub>t</sub>

Î¸=Î¸âˆ’Î·â‹…v<sub>t</sub>

Where:

- v<sub>t</sub> is the momentum term at time t,
- g<sub>t</sub> is the current gradient,
- Î² is the momentum coefficient (typically around 0.9),
- Î· is the learning rate,
- Î¸ represents the parameters (weights) being updated.

In this formulation, the **momentum term** v<sub>t</sub> is updated by combining a portion of the previous momentum v<sub>tâˆ’1</sub>â€‹ and the current gradient g<sub>t</sub>. The parameter Î²\\betaÎ² controls how much of the previous momentum is retained versus the current gradientâ€™s influence:
...
### 3\. RMSprop: Addressing AdaGradâ€™s Diminishing Learning Rate
...
- E\[g<sup>2</sup>\]<sub>t</sub>â€‹ represents the moving average of squared gradients at time ttt,
- Î³ is the decay rate for the moving average, typically set around 0.9.
...
- m^<sub>t</sub>â€‹ and v^<sub>t</sub>â€‹ are the bias-corrected estimates of m<sub>t</sub> and v<sub>t</sub>,
- Î²<sub>1</sub> and Î²<sub>2</sub> are decay rates for the moment estimates (default values are 0.9 and 0.999, respectively).
...
2.  **Adjust Learning Rates**: While Adam performs well with default settings, specific tasks may benefit from adjusting Î·, Î²<sub>1</sub>, and Î²<sub>2</sub>â€‹.
...
To illustrate the basics of model fitting, letâ€™s walk through a simple example of fitting a **linear model** to data, then discuss how using overly complex models can lead to **overfitting**.
...
When we fit models to data, the complexity of the model should align with the complexity of the data. If we use a model thatâ€™s much more complex than the underlying data, we risk **overfitting**, where the model learns not only the underlying trend but also the noise or random fluctuations in the data.
...
To understand overfitting, letâ€™s explore how **training loss** and **validation loss** behave over time:
...
To prevent overfitting and ensure that our model captures only the relevant patterns, itâ€™s important to:
...
- **Regularize the Model if Necessary**: Use regularization techniques, which weâ€™ll cover later, to penalize overly complex models, especially in cases where simpler models may not suffice.
...
In machine learning, understanding the **bias-variance tradeoff** is essential to finding the right balance between model complexity and performance. The **total error** in a modelâ€™s predictions can be dissected into three main components: **bias error**, **variance error**, and **irreducible error**.

### Bias

**Bias** refers to the error introduced by assumptions made in the model as it tries to approximate the true function underlying the data. In simple terms, bias represents how closely the modelâ€™s predictions align with the actual values on average.
...
**Variance** measures how much the modelâ€™s predictions fluctuate with different training sets. High-variance models are sensitive to the specific data they are trained on, often producing a wide spread of predictions across different samples.
...
Balancing **bias** and **variance** is crucial because increasing model complexity typically reduces bias but increases variance, and vice versa. This balance determines the modelâ€™s **generalization capability** on unseen data:
...
When we have complex models with many parameters, there's a risk of the model becoming too good at fitting the training data, capturing noise along with the underlying pattern. Regularization techniques help manage this by slightly increasing the modelâ€™s bias but significantly reducing variance. This adjustment helps avoid **overfitting** while guiding the model toward a more generalized form.
...
2.  **Early Stopping
    **With early stopping, we halt training as soon as the validation loss stabilizes, producing a model that captures the main trends without overfitting. The modelâ€™s predictions are smoother and more stable across the training data range.
...
2.  **Challenges with Momentum-Based Optimizers**
    - Adam, and other momentum-based optimizers, maintain **momentum terms**â€”which store historical gradients to smooth out updates.
...
3.  **Weight Decay: An Improved Approach**
    ...
    - This approach prevents the interaction with the momentum term, ensuring that the L2 penalty doesnâ€™t accumulate in unintended ways.
    - When using **AdamW** (Adam with weight decay), weight decay is implemented after the gradient computation, achieving a stable regularization effect without interfering with Adamâ€™s momentum dynamics.
4.  **Practical Implementation**
    - In frameworks like PyTorch, using AdamW allows for easy inclusion of weight decay by specifying a weight_decay parameter, effectively simplifying the regularization process while preserving the optimizerâ€™s efficiency.
...
- **Mechanism**: Dropout randomly ignores a subset of neurons during training. For each forward pass, a fraction (e.g., 50%) of neurons in a given layer are â€œdropped outâ€ or temporarily deactivated.
...
- **Parameter Behavior**: Dropout creates a â€œcloudâ€ of parameter values, representing the distributed learning across various paths. This can look like a dispersed range of parameter magnitudes, though the precise interpretation varies.
...
Initially, batch normalization was proposed to address **internal covariate shift**â€”the idea that layer inputs change distribution during training due to parameter updates in preceding layers, causing instability and necessitating frequent re-adjustment of learning rates.
...
Although the precise reasons behind BatchNormâ€™s effectiveness are still under investigation, its practical benefits for accelerating and stabilizing training make it a staple in neural network architectures, particularly for complex models. By understanding and implementing BatchNorm correctly, you can enhance model performance, reduce training time, and improve the network's generalization capabilities across diverse tasks and data distributions.
...
In classification tasks, models often output probabilities indicating confidence in their predictions. Ideally, the model should avoid **overconfident predictions**â€”e.g., assigning probabilities near 1 or 0 for every predictionâ€”unless it has substantial evidence for such certainty.
