...
## Source
- Raw source: `Raw/Sources/lecture_12_dna_language_models.md`
- Supporting source: `Raw/Files/lecture_12_dna_language_models.md`
- Existing related pages: [[mlcb-2024-computational-biology]], [[mlcb-complete-study-guide]], [[mlcb-methods-map]]
- Last updated: 2026-05-24

## Executive Summary
Lecture 12 - DNA Language Models develops splicing, 1D CNNs, one-hot DNA, DNABERT, Nucleotide Transformer, SegmentNT, UNets, Hyena, EvoDNA, and sequence-to-expression prediction. This page intentionally preserves substantial source detail so a future LLM can reason from the wiki without immediately reopening the raw transcript.
...
- Intro to DNA language models
- RNA Splicing Models and Splice AI
- Understanding Convolutions in Neural Networks and Their Application
- 1D Convolutions for DNA Sequence Analysis
- SpliceAI: Advancements in RNA Splicing Prediction
- DNA Foundation Models: Training and Applications
- DNABERT Architecture: Tokenization, Embedding, and Attention Layers
- The Nucleotide Transformer: A Deep Foundation Model for DNA Analysis
- SegmentNT: Leveraging the Nucleotide Transformer with UNet Architecture for Genomic Segmentation Tasks
- Hyena Models: Efficient Long-Range Processing in DNA Sequence Analysis
...
TodayÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s session dives into **DNA language models**, with a continued focus on Transformer-based approaches and an exploration of **convolutional neural networks (CNNs)** as a mainstay model type used for DNA sequence analysis.
...
In this chapter, weÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ll explore how these models apply to DNA, their respective strengths, and how they work together to interpret complex genomic data.
...
1.  **One-Hot Encoding**: DNA sequences are first converted into one-hot encoded vectors. Each nucleotide (A, T, C, G) is represented by a unique vector with a single '1' in the corresponding position and '0's elsewhere.
    - **Example**: A DNA sequence "ATCG" would convert into a matrix where each row represents a nucleotide in one-hot encoding:
        - A ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ \[1, 0, 0, 0\]
        - T ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ \[0, 1, 0, 0\]
        - C ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ \[0, 0, 1, 0\]
        - G ÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢ \[0, 0, 0, 1\]
...
2.  **Convolutional Layers and Kernel Parameters**:
    ...
    - **Width (W)**: Defines the number of nucleotides considered in each convolution operation. For SpliceAI, the width is typically set to 11 nucleotides, capturing enough local context to account for one full DNA helix turnÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âa meaningful segment for DNA-binding proteins.
...
4.  **Skip Connections and Batch Normalization**:
    - Skip connections (also called residual connections) enable the model to retain initial input information by adding outputs from earlier layers directly to later layers. This structure stabilizes training and enhances SpliceAIÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s ability to recognize both local and long-range signals.
...
- **Rich Internal Representation**:
    - Once trained, DNA foundation models have embeddings or internal representations that encapsulate a wide range of DNA information, not limited to just the immediate task. These representations are thought to contain:
        - **Splicing information** (similar to the SpliceAI modelÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s focus)
...
After completing the pre-training task, the model can generate **rich embeddings** for each position within a DNA sequence. These embeddingsÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âhigh-dimensional vectors representing each nucleotide's context and biological significanceÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âform the basis for transfer learning, where the Nucleotide Transformer can be fine-tuned for specialized tasks.

### Transfer Learning and Fine-Tuning: Versatility in Genomic Tasks

Once pre-trained, the Nucleotide TransformerÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s parameters are mostly **frozen**, and the model is used as a feature extractor. Its embeddings capture a wealth of information about DNA sequences, which can then be leveraged by secondary networks for specific genomic tasks. The model excels in:

1.  **Binary Classification Tasks**: For example, it can determine whether a sequence acts as a **promoter** (indicating the starting region of a gene) or not. During fine-tuning, a small classifier network is trained on the Nucleotide Transformer embeddings to classify sequences. Since the majority of the TransformerÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s parameters are frozen, fine-tuning only requires adjusting the classifier network parameters, making this a computationally efficient process.
...
- **Transferable Embeddings for Downstream Applications**: The Nucleotide TransformerÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s embeddings serve as rich features that can be applied to numerous prediction tasks, enabling robust **transfer learning**. This property minimizes the need for task-specific model re-training, which is especially useful in applications like gene expression prediction, variant annotation, and regulatory element identification.
- **Interpretability via Attention Mechanisms**: As with DNABERT, the attention layers within the Nucleotide Transformer allow for **interpretability**. Attention maps can be visualized to highlight areas of focus within the DNA sequence, often aligning with known functional sites, such as transcription factor binding sites or regulatory elements. This transparency aids researchers in validating the modelÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s predictions against biological knowledge.
...
3.  **Skip Connections**: An essential feature of the UNet is its **skip connections** (also known as residual connections). These connections enable the model to **retain high-resolution information** from the original input sequence by directly passing it to the corresponding upsampled layers. This design choice preserves essential spatial and contextual information that may be lost during downsampling, improving the modelÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s ability to generate accurate segmentations. Skip connections also play a critical role in **denoising** tasks by smoothing out noise and enhancing the clarity of structural details.
...
**EvoDNA** is an extension of the Hyena model architecture, introduced as another **DNA foundation model** that builds on the efficient information processing principles of Hyena. EvoDNA incorporates **attention layers** alongside the **Hyena architectureÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s convolutional and gating mechanisms**. This hybrid model, referred to as a **striped Hyena model**, integrates **selective attention** with HyenaÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s efficient processing structure to enhance performance across a range of genomic tasks.
...
### BorzoiÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s Contribution to Gene Expression Prediction
...
You should be able to explain how splicing, 1D CNNs, one-hot DNA, DNABERT, Nucleotide Transformer, SegmentNT, UNets, Hyena, EvoDNA, and sequence-to-expression prediction connects a biological problem to a computational representation, model, or algorithm.
...
## Key Takeaways
- splicing, 1D CNNs, one-hot DNA, DNABERT, Nucleotide Transformer, SegmentNT, UNets, Hyena, EvoDNA, and sequence-to-expression prediction is part of the MLCB modeling arc.
