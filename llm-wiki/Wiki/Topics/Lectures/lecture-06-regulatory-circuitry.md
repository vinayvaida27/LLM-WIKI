...
## Source
- Raw source: `Raw/Sources/lecture_06_regulatory_circuitry.md`
- Supporting source: `Raw/Files/lecture_06_regulatory_circuitry.md`
- Existing related pages: [[mlcb-2024-computational-biology]], [[mlcb-complete-study-guide]], [[mlcb-methods-map]]
- Last updated: 2026-05-24

## Executive Summary
Lecture 6 - Regulatory Circuitry develops enhancer-promoter looping, HMM training, chromatin compartments, Hi-C, TADs, eQTLs, enhancer modules, motifs, and disease circuitry. This page intentionally preserves substantial source detail so a future LLM can reason from the wiki without immediately reopening the raw transcript.
...
- Introduction
- Epigenomics review: signals, mapping, HMMs
- Parsing, scoring, Viterbi decoding, optimal path
- Posterior decoding, Forward Algorithm
- Learning HMM parameters: Supervised
- Learning HMM parameters: Unsupervised (Best Path, Viterbi)
- Learning HMM parameters: Unsupervised (All Paths, Baum-Welch)
...
In this lecture, weÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ll touch on concepts from previous weeks, such as **expression analysis**, **single-cell analysis**, **sequential data in hidden Markov models (HMMs)**, and **epigenomics**, as we integrate all of these into todayÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s topic of **regulatory genomics**. We'll also get ready to move on to **biological networks** next time.
...
This approach is critical because **genomic features**, such as enhancers and promoters, tend to occur near each other. For instance, itÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s far more likely to find an **enhancer** near a **transcribed region** rather than in the middle of **repressed regions**. These relationships are encoded in the **transition probability matrix** of the HMM, which tells us the probability that the next state at position iii is LLL given that the state at position iÃƒÂ¢Ã‹â€ Ã¢â‚¬â„¢1i-1iÃƒÂ¢Ã‹â€ Ã¢â‚¬â„¢1 was KKK. This captures transitions such as **enhancer to promoter**, **promoter to transcribed region**, and so on.
...
For instance, while the best path might pass through an **enhancer** state at position 523, many other high-probability paths could pass through the **promoter** state at that same position. In this case, itÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s not enough to focus only on the best path. Instead, we should sum over **all possible paths** that pass through the enhancer and compare them to the sum of all paths that pass through the promoter at position 523. This gives us a better understanding of the likelihood of being in one state versus another at that specific position.
...
A small adjustment is needed: we donÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢t want to count the **emission probabilities** twice, so we include them in the Forward Algorithm but exclude them from the Backward Algorithm.

Once weÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ve completed these two steps, we can calculate the **posterior probability** of being in a given state at any position. This is known as **Posterior Decoding**, which looks at the **posterior probability** of being in a state after observing the entire sequence. This method doesnÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢t necessarily give us a valid transition path, but it allows us to determine the most likely state at each individual position.
...
In a **supervised learning** context, we already have labeled data, meaning we know both the **observations** (the sequence of emitted symbols) and the **hidden states**. With this information, itÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s straightforward to estimate both the **emission** and **transition** probabilities for the Hidden Markov Model (HMM).
...
Thus, the **emission probability** of **A** from **P** is 23\\frac{2}{3}32ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â¹. You repeat this process for all symbols and states to build the full **emission probability matrix**.
...
For example, if you observed 5 transitions from state **B**, and one of those transitions led to **P**, the **transition probability** from **B** to **P** would be 15\\frac{1}{5}51ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â¹.

Similarly, if you were in **P** three times and transitioned to **B** once, the **transition probability** from **P** to **B** would be 13\\frac{1}{3}31ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â¹. If you stayed in **P** twice, the **transition probability** from **P** to **P** would be 23\\frac{2}{3}32ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â¹.

This is a **maximum likelihood estimation** approach, which works well when thereÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s plenty of data.

### Addressing Zero Counts

One challenge in this supervised approach arises when you donÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢t observe certain transitions or emissions. If you only have a small dataset, some transitions might never occur, leading to **zero probabilities**, which can skew the model. To address this, we introduce **pseudo counts**, which add a small number (like 1) to each count to avoid zeros in the probabilities. This technique smooths the estimates and ensures the model remains robust, even with limited data.
...
Both of these methodsÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â**correlation-based** and **genetics-based linking**ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âallow for a more comprehensive understanding of how **non-coding genetic variation** impacts gene regulation and ultimately contributes to different phenotypes, including disease states.
...
You should be able to explain how enhancer-promoter looping, HMM training, chromatin compartments, Hi-C, TADs, eQTLs, enhancer modules, motifs, and disease circuitry connects a biological problem to a computational representation, model, or algorithm.
...
## Key Takeaways
- enhancer-promoter looping, HMM training, chromatin compartments, Hi-C, TADs, eQTLs, enhancer modules, motifs, and disease circuitry is part of the MLCB modeling arc.
