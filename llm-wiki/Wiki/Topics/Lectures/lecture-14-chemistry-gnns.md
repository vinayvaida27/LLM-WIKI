...
## Source
- Raw source: `Raw/Sources/lecture_14_chemistry_gnns.md`
- Supporting source: `Raw/Files/lecture_14_chemistry_gnns.md`
- Existing related pages: [[mlcb-2024-computational-biology]], [[mlcb-complete-study-guide]], [[mlcb-methods-map]]
- Last updated: 2026-05-24

## Executive Summary
Lecture 14 - Chemistry GNNs develops SMILES, SELFIES, SMARTS, InChI, Morgan fingerprints, molecular graphs, QSAR, geometric deep learning, message passing, pooling, and virtual nodes. This page intentionally preserves substantial source detail so a future LLM can reason from the wiki without immediately reopening the raw transcript.
...
- Encoding Molecules as SMILES Strings
- Morgan fingerprints: Encoding Molecular Structure for Machine Learning
- Molecular graphs: Advanced Representation for Chemical Structures
- Applications of molecular graphs, and review of CNNs
- Geometric deep learning: Extending Deep Learning to Complex Structures
- Overview of Graph Neural Networks (GNNs): Structure, Workflow, and Applications
- Message passing in Graph Neural Networks (GNNs)
- Node Updates in a Basic Graph Neural Network (GNN)
- Utilizing the Output of a GNN
...
- **Examples**: For **sodium ion** (NaÃƒÂ¢Ã‚ÂÃ‚Âº), SMILES would be \[Na+\], and for **chloride ion** (ClÃƒÂ¢Ã‚ÂÃ‚Â»), \[Cl-\].
- **Multiple Charges**: If an atom has multiple charges, the count is specified next to the charge symbol (e.g., \[Fe+2\] for FeÃƒâ€šÃ‚Â²ÃƒÂ¢Ã‚ÂÃ‚Âº).
...
Morgan fingerprints are a widely-used method in cheminformatics for encoding molecular information into **fixed-length vectors**. By hashing molecular substructures to predefined bit positions, they capture **topological and structural details** of a molecule. These fingerprints are particularly useful for tasks like **quantitative structure-activity relationship (QSAR)** modeling, which aims to predict properties such as hydrophobicity, solubility, and potential bioactivity. TheyÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢re also effective for **high-throughput screening** in drug discovery.
...
Each atom and substructure are hashed into the fingerprint, resulting in a bit vector unique to ethanol's structure. This encoding captures local molecular features, providing a useful summary of the moleculeÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s structure.
...
4.  **Weighted Graphs**: Although not commonly used in basic molecular graphs, **weighted edges** can be introduced to indicate bond strengths, interaction intensities, or other prior knowledge about the moleculeÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s structure or behavior.
...
**Molecular graphs** are indispensable in various stages of **drug discovery and development**, enabling researchers to capture and manipulate the full structure of molecules with precision. HereÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s how molecular graphs can significantly enhance drug development workflows:
...
To understand **graph neural networks** (GNNs) more intuitively, itÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s helpful to consider the structure and function of **convolutional neural networks** (CNNs), which are widely used in image processing.
...
1.  **Designing Invariant or Equivariant Layers**: The structure of neural network layers can be crafted so that transformations are inherently handled. For instance, in a **graph neural network (GNN)**, layers can be designed so that they aggregate information based on node connections without depending on the nodeÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s spatial arrangement.
2.  **Penalty-Based Regularization**: Another approach involves adding a penalty in the **objective function** if the modelÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s output changes undesirably under transformations. This incentivizes the model to learn invariant or equivariant features as needed.
...
In molecular modeling, **geometric deep learning** provides tools that are directly applicable to **molecular graphs**, where the relationships between atoms (nodes) and bonds (edges) define the moleculeÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s structure. By treating each atom as a node and each bond as an edge, we can apply geometric deep learning principles to predict molecular properties, interactions, and behavior in a spatially-aware manner.
...
2.  **Message Passing Through GNN Layers**: After embedding, the GNN performs a **series of message-passing steps** across several GNN layers. This process is akin to the way convolutional layers in CNNs aggregate information from neighboring pixels, but in GNNs, the information is aggregated from connected nodes in the graph structure.
    ...
    - **Multiple Layers**: Information is passed across multiple layers, effectively allowing each node to ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œseeÃƒÂ¢Ã¢â€šÂ¬Ã‚Â further in the graph with each layer. For example, after two layers, each node can incorporate information from its two-hop neighborhood.
3.  **Final State and Output Generation**: Once the graph has been processed through the GNN layers, the nodes reach a **final state**, which encapsulates the learned information from their surroundings. This state can then be used to generate predictions:
    - For node-level tasks, each nodeÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s final state serves as the basis for the prediction.
...
To illustrate, letÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s consider a **hypothetical molecular prediction task** using a GNN:
...
- **Convolution on Graphs**: In CNNs, convolution is performed in fixed grids; in GNNs, the convolution operates over the neighborhood of each node based on the edges. This local aggregation of features enables each node to learn from its neighbors while respecting the graphÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s inherent structure.
...
Each GNN layer involves several key steps in the message-passing process. LetÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s break them down with a focus on how this affects the embedding and update process of each node.
...
1.  **Message Creation**: For each node in the graph, GNNs create messages from its neighbors. This is typically done by taking the **latent vector (hidden representation)** of each neighboring node and transforming it using a **learnable weight matrix**. This transformation allows each node to convey information in a way that is meaningful to the task.
    - For example, if weÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢re looking at the **yellow node** in a graph, it receives messages from its connected neighbors (e.g., the pink and gray nodes). Each neighborÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s latent vector is transformed (e.g., by multiplying it by a weight matrix), producing a **message vector** for each connection.
...
3.  The choice of aggregation method impacts the GNNÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s behavior and performance. **Normalization** is often necessary, especially if there are nodes with widely varying numbers of connections, to ensure that highly connected nodes do not disproportionately affect the final representation.
4.  **Permutation Invariance and the Challenges of Directionality**: In a GNN, there is no inherent ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œdirectionÃƒÂ¢Ã¢â€šÂ¬Ã‚Â or position for neighbors (unlike grids in CNNs, where pixels have specific positions relative to each other). This absence of structure in node ordering requires the GNN to be **permutation invariant**ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âthe order in which messages from neighbors are received should not affect the result.
...
5.  **Update Function**: After aggregating messages from neighbors, each node updates its latent vector. This step combines the nodeÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s **current latent vector** with the **aggregated message vector**. The update function often consists of multiplying the nodeÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s latent vector by another learnable weight matrix and combining it with the aggregated messages (e.g., by adding or concatenating). This updated latent vector represents the nodeÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s new state, enriched with contextual information from its neighborhood.
...
Suppose weÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢re analyzing a molecular graph to predict the **solubility** of a compound. In this case:
...
- After applying the aggregation function (e.g., mean aggregation), the carbon atomÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s latent vector now reflects an ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œaverageÃƒÂ¢Ã¢â€šÂ¬Ã‚Â influence from its neighbors.
...
In summary, **message passing** is the core process that enables GNNs to learn meaningful representations of nodes, edges, and entire graphs. By carefully designing each stepÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âmessage creation, aggregation, and updatingÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬ÂGNNs can adapt to the complexities of molecular graphs, providing robust insights into properties and behaviors across diverse chemical structures.
...
In a basic Graph Neural Network (GNN), node updates occur as part of each layerÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s **message-passing process**. This update mechanism is crucial because it allows each node to build an increasingly sophisticated representation by iteratively integrating information from its neighbors.

### Step-by-Step Breakdown of the Update Mechanism

1.  **Initialize Node Embeddings**: Each node uuu in the graph has an initial embedding, denoted as Hu(0)H^{(0)}\_uHu(0)ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â¹, which typically comes from input features or an embedding layer.
2.  **Compute Latent Representations**: For each layer kkk in the GNN, we update the latent representation of each node based on the information from its neighbors. At layer kkk, the latent state of node uuu, denoted H<sub>u</sub><sup>(k)</sup>ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â¹, is computed by incorporating messages from neighboring nodes. The general update equation is as follows:
...
6.  Therefore, for many practical applications, itÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s beneficial to use a limited number of GNN layers (e.g., 2-4 layers). This approach balances computational efficiency with the depth of information each node can incorporate.
...
4.  **Update with Non-Linearity**: Update each nodeÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s latent vector by combining its previous state with the aggregated neighbor messages and applying a non-linear function.
...
Once weÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ve completed the layers and message-passing steps in a **Graph Neural Network (GNN)**, weÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢re left with a graph where each node has a final latent representationÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âa vector that encodes information from both the node's features and those of its neighboring nodes. But how do we transform these node-level representations into useful predictions, especially when our objective may vary widely, from classifying individual nodes to predicting properties for the entire graph?
...
1.  **Node-Level Predictions**:
    - For tasks where weÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢re interested in classifying or predicting attributes for individual nodes, we can directly use each node's final latent representation.
...
- **Mean Aggregation**:
    - This normalizes the aggregation by taking an average of node vectors, ensuring that the graph size doesnÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢t impact the result. ItÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s particularly useful when dealing with graphs of varying sizes, as it adjusts for node count disparities.
- **Max Pooling**:
...
- **Attention Mechanisms**:
    - Attention-based mechanisms allow us to weigh each nodeÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s contribution differently based on its importance to the task. The model learns these weights, which adaptively highlight critical nodes, offering a powerful way to prioritize certain graph regions. For example, in a molecule, attention mechanisms can weigh critical functional groups more heavily when predicting chemical properties.
- **Global Nodes or Virtual Nodes**:
    - In some advanced GNNs, a ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œglobalÃƒÂ¢Ã¢â€šÂ¬Ã‚Â or ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œvirtualÃƒÂ¢Ã¢â€šÂ¬Ã‚Â node is introduced, connected to all other nodes. During message-passing, this node aggregates information from across the graph, acting as a central hub. By the end, the virtual nodeÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s latent vector becomes the graph-level representation, capturing the entire networkÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s features.
...
- For **regression tasks** (e.g., estimating a moleculeÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s solubility), the vector is fed into a regression model to predict a continuous value.

### Recap and Considerations

Using the output of a GNN involves multiple considerations, including the nature of the task, the structure of the data, and the size and diversity of the graphs. Each choice in the processÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âfrom aggregation technique to model typeÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âaffects the GNN's ability to make accurate, generalizable predictions.
...
You should be able to explain how SMILES, SELFIES, SMARTS, InChI, Morgan fingerprints, molecular graphs, QSAR, geometric deep learning, message passing, pooling, and virtual nodes connects a biological problem to a computational representation, model, or algorithm.
...
## Key Takeaways
- SMILES, SELFIES, SMARTS, InChI, Morgan fingerprints, molecular graphs, QSAR, geometric deep learning, message passing, pooling, and virtual nodes is part of the MLCB modeling arc.
