...
## Source
- Raw source: `Raw/Sources/lecture_08_intro_to_protein_structure.md`
- Supporting source: `Raw/Files/lecture_08_intro_to_protein_structure.md`
- Existing related pages: [[mlcb-2024-computational-biology]], [[mlcb-complete-study-guide]], [[mlcb-methods-map]]
- Last updated: 2026-05-24

## Executive Summary
Lecture 8 - Intro to Protein Structure develops protein structure, PDB, DNA binding, allostery, crystallography, NMR, Cryo-EM, folding funnels, Ramachandran plots, contact maps, and RMSD. This page intentionally preserves substantial source detail so a future LLM can reason from the wiki without immediately reopening the raw transcript.
...
- Using PyMOL for Structural Analysis of Transcription Factors
- Transcription factor-DNA binding and functional specificity
- Experimental methods for determining structure
- Protein structure: conformations and Folding
- The protein folding problem: Levinthal's Paradox and the Energy Landscape
- Protein secondary structure
- Protein tertiary structure: From Sequence to 3D Configuration
- Comparing protein structures: Quantitative and Structural Alignment Methods
...
This session serves as a gateway into **protein structure fundamentals** while setting the stage for upcoming discussions on **deep learningÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s transformative role** in protein structure prediction and the broader implications for biology and computational science.
...
**Structural biology** is the study of molecular structures to understand their function, particularly focusing on proteins. It involves both analyzing **pre-determined structures** to identify patterns (such as common features across enzymes) and **determining structures experimentally or computationally** to uncover specific details. The central challenge in structural biology, particularly for proteins, has been how to transition from a linear **protein sequence (derived from genomic data)** to a **three-dimensional structure**. Such a structure can then shed light on the moleculeÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s function, which is essential for understanding biological processes.
...
In structural biology, **understanding the relationship between a proteinÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s structure and its function** is crucial, and PyMOL is a valuable tool for visualizing these interactions. Here, we focus on a **transcription factor** from the **LacI family**, commonly found in prokaryotes, which features a **helix-turn-helix motif** for DNA binding. The goal is to explore how this protein binds DNA and controls gene expression, using PyMOL to uncover key structural details.
...
- In **PurR** and similar proteins, three critical residues in the recognition helix protrude into the DNAÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s major groove, where they make **specific contacts** with the DNA bases, determining the proteinÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s binding specificity.
...
This hierarchical organizationÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âfrom specific amino acid contacts in DNA-binding helices to operon-level gene layoutÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âillustrates how structural elements at multiple scales collectively **determine the function** of transcription factors and their role in cellular regulation.
...
In **structural biology**, accurately determining the three-dimensional structure of proteins and protein complexes is crucial for understanding their function. Three primary experimental techniquesÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â**X-ray crystallography**, **nuclear magnetic resonance (NMR)**, and **cryogenic electron microscopy (cryo-EM)**ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âare widely used to achieve this, each with distinct advantages, limitations, and suitability depending on the protein's size, stability, and the environment in which it functions.
...
However, **phase information**ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âcritical for constructing the electron density mapÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âis lost during diffraction. To recover this phase information, researchers use techniques such as:
...
The resulting structure is characterized by its **resolution**, a key metric of quality. Higher resolution (indicated by smaller numbers, typically below 3 ÃƒÆ’Ã¢â‚¬Â¦ngstroms) means more detailed and precise atomic positions. Some modern structures are determined at **sub-angstrom resolutions** (below 1 ÃƒÆ’Ã¢â‚¬Â¦), allowing researchers to see nearly every atomÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s position in exquisite detail.
...
1.  **Phi (ÃƒÂÃ¢â‚¬Â )**: This is the angle between the **nitrogen atom** and the **alpha carbon** (CÃƒÅ½Ã‚Â±). The ÃƒÂÃ¢â‚¬Â  angle allows rotation around the bond connecting the nitrogen of one amino acid to the alpha carbon of the same amino acid.
2.  **Psi (ÃƒÂÃ‹â€ )**: This is the angle between the **alpha carbon** and the **carbonyl carbon** of the same amino acid. Rotation around this bond influences the orientation of the backbone downstream of the alpha carbon.
3.  **Omega (ÃƒÂÃ¢â‚¬Â°)**: This bond is between the **carbonyl carbon** of one amino acid and the **nitrogen** of the next amino acid in the chain. The ÃƒÂÃ¢â‚¬Â° angle usually adopts a **trans configuration** (180Ãƒâ€šÃ‚Â°) due to steric hindrance, which is generally stable. However, when **proline** is present, the ÃƒÂÃ¢â‚¬Â° angle may adopt a **cis configuration**, introducing a specific type of rigidity.

While each amino acid has unique side chains that influence these angles, the **phi and psi angles** are the most influential in defining the overall **three-dimensional shape** of the protein, dictating how it will fold and pack into its final structure. A complete description of a proteinÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s shape includes specifying these angles for each residue in the chain.

### Conformational Preferences: Energy Minimization

Despite the seeming **continuum of rotational possibilities** (from 0Ãƒâ€šÃ‚Â° to 360Ãƒâ€šÃ‚Â°) for each bond, proteins tend to adopt only a few **low-energy conformations**. This preference arises because the **spatial arrangement of atoms** naturally favors configurations that **minimize steric clashes** and maximize stability. For example, if we examine the simple molecule **ethane**, where two large atoms are attached, the lowest energy conformation positions these bulky groups as far from each other as possible. This concept also applies to larger, more complex protein structures, where **rotational freedom** is restricted to avoid high-energy, sterically unfavorable arrangements.
...
### Christian AnfinsenÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s Hypothesis: Energy Minimization

In the 1950s, **Christian Anfinsen** proposed that a proteinÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s folded conformation represents its **global free energy minimum**ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âthe lowest energy state it can achieve in its given environment. This hypothesis suggested that proteins would naturally adopt their **native conformation** because this structure is thermodynamically the most stable. Essentially, the idea was that, given the right conditions, the sequence alone contains all the information needed for a protein to fold into its functional form.

### LevinthalÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s Paradox: The Search Problem

In 1969, **Cyrus Levinthal** articulated a challenge to AnfinsenÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s hypothesis, which became known as **LevinthalÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s Paradox**. He observed that if proteins were to search through all possible conformations to find the one with the lowest energy, it would take **longer than the age of the universe** for a protein to reach its folded state. For a small protein of 100 amino acids, the possible conformations number around 1010010^{100}10100ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âan astronomically large space to search.

Yet, in practice, many proteins fold **within milliseconds to seconds**. This apparent contradictionÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âbetween the need for exhaustive conformational sampling and the rapidity of actual foldingÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âhighlighted that proteins must employ an **efficient search strategy** to locate their folded state, circumventing a brute-force approach.

### Resolving LevinthalÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s Paradox: The Folding Funnel and Energy Landscape Theory

The resolution to LevinthalÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s Paradox lies in the concept of the **energy landscape**, often visualized as a **folding funnel**. Rather than randomly sampling every possible conformation, proteins fold through a series of **thermodynamically favorable intermediate states**:
...
In 1998, researchers discovered that the **contact order**ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âthe **average sequence distance between amino acids that interact in the folded structureÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âcorrelates with folding speed**. Proteins with **low contact order** (i.e., where interacting residues are close in sequence) tend to fold faster than those with high contact order. This finding supports the idea that **local interactions** form quickly, helping to stabilize portions of the structure early in the folding process and guiding the overall conformation.
...
The **Ramachandran plot** is a powerful tool in structural biology, visualizing the **dihedral angles (phi and psi)** of amino acid residues in proteins. By plotting the phi (ÃƒÂÃ¢â‚¬Â ) and psi (ÃƒÂÃ‹â€ ) angles for residues across many proteins, we observe distinct regions where these angles are more favorable, reflecting the **limited conformational space** due to **steric hindrance** and **backbone rigidity**:
...
- These sheets play a crucial role in structural scaffolding and are often found in the core of proteins, where they contribute to the **rigid, planar stability** necessary for maintaining the proteinÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s overall shape.
...
3.  **Modern Predictive Techniques**:
    - Today, advances in **machine learning** and **deep learning**ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âexemplified by models like **AlphaFold**ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Âhave significantly improved **three-dimensional protein structure prediction**, often rendering isolated secondary structure prediction unnecessary.
...
These contact maps reveal not only **secondary structure elements** but also insights into tertiary structure by displaying which residues interact across the proteinÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s 3D conformation.
...
- By analyzing **multiple sequence alignments (MSAs)** across homologous proteins, scientists observe that certain amino acid pairs change in tandem. When one amino acid in an interaction changes, the paired amino acid may change to maintain the interactionÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s stability.
...
1.  **Rosetta**:
    ...
    - RosettaÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢s **fragment-based methodology** allowed for plausible predictions even for regions without direct homologs, setting a new standard in structure prediction and revealing the utility of leveraging **empirical data** in structural biology.
...
- **Interpretation**: Lower RMSD values indicate better alignment (closer structural similarity), with values below **2 ÃƒÆ’Ã¢â‚¬Â¦** often reflecting very similar structures.
...
You should be able to explain how protein structure, PDB, DNA binding, allostery, crystallography, NMR, Cryo-EM, folding funnels, Ramachandran plots, contact maps, and RMSD connects a biological problem to a computational representation, model, or algorithm.
...
## Key Takeaways
- protein structure, PDB, DNA binding, allostery, crystallography, NMR, Cryo-EM, folding funnels, Ramachandran plots, contact maps, and RMSD is part of the MLCB modeling arc.
